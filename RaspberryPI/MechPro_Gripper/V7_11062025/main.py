...
# Anpassung der Pumpensteuerung                                                       // 22.05.2025       // Johann Spielvogel
# Anpassung der Ansteuerung des Servos für die Jellyfish Aufgabe					  // 11.06.2025       // Johann Spielvogel
#
#
#
#
...
def start_pumpe():
    motor_in1.value(1)
    motor_in2.value(0)
    motor_pwm.duty_u16(65535)  # Ca. 100% PWM-Leistung -> 5V
...
#Servo_4---------------------------------------------
# Funktion zum Speichern der Servo-Position in eine Datei
def save_servo_4_position(angle):
    try:
        with open("Servo_4_pos.txt", "w") as f:
            f.write(str(angle))  # Schreibe die Position in die Datei
        print(f"Position {angle} gespeichert.")
    except Exception as e:
        print(f"Fehler beim Speichern der Position: {e}")
...
# Funktion zum Setzen der Servo-Position
def set_servo_4_angle(angle):
    global servo_4_angle
    # Erlaubter Bereich: 10–146 (beachte: Bereich geht ueber 0 hinweg!)
    if 10 <= angle <= 146:
        duty_ns = int(500_000 + (angle / 180) * 2000_000)
        servo_4.duty_ns(duty_ns)
        servo_4_angle = angle
        save_servo_4_position(angle)
        print(f"-> Servo_4 auf {servo_4_angle}°")
    else:
        print(f"-> Ungueltiger Winkel fuer Servo_4: {angle}° (nur 10-146 erlaubt)")

    
# Funktion zum Laden der gespeicherten Servo-Position
def load_servo_4_position():
    try:
        with open("Servo_4_pos.txt", "r") as f:
            return int(f.read())  # Gibt die gespeicherte Position zurueck
    except:
        print("Keine gespeicherte Position gefunden. Setze auf 90°.")
        return 90  # Wenn keine Position vorhanden ist, starte bei 90°
...
# Setze Servo auf Position OHNE Speicherung
def move_servo_4_without_saving(angle):
    global servo_4_angle
    angle = max(0, min(180, angle))  # Begrenzung auf gueltigen Bereich
    duty_ns = int(500_000 + (angle / 180) * 2000_000)
    servo_4.duty_ns(duty_ns)
    servo_4_angle = angle
    print(f"-> Servo_4 initialisiert auf {servo_4_angle}° (ohne Speichern)")

# Lade letzte gespeicherte Position
servo_4_angle = load_servo_4_position()
print(f"Letzte gespeicherte Position: {servo_4_angle}°")

# Nur PWM aktivieren – Servo bleibt auf letzter Position steht
move_servo_4_without_saving(servo_4_angle)

servo_4_mode = "stop"  # "links", "rechts", "stop"
servo_4_step = 4       # Schrittweite in Grad; default: 2
servo_4_delay = 70     # Zeit in ms zwischen Schritten; default: 100
last_servo_4_move = time.ticks_ms()
servo_4_target_angle = None  # Zielwinkel, falls vorhanden


#Allgemein: Servos 1-4
...
#         new_angle = servo_3_angle + delta
#         if 70 <= new_angle <= 150:
#             servo_3_target_angle = new_angle
#         else:
#             print(f"-> Servo_3 Zielwinkel {new_angle}° außerhalb des erlaubten Bereichs!")
    
    elif servo_number == 4:
        global servo_4_target_angle
        new_angle = servo_4_angle + delta
        if 10 <= new_angle <= 146:
            servo_4_target_angle = new_angle
        else:
            print(f"-> Servo_4 Zielwinkel {new_angle}° außerhalb des erlaubten Bereichs!")
...
pumpe_aktiv = False  # Globale Variable für Pumpenstatus
pumpe_startzeit = 0
while True:
...
#             last_servo_3_move = now
    ...
    # Servo-4 Bewegung nach Modus
    if servo_4_mode != "stop":
        now = time.ticks_ms()
        if time.ticks_diff(now, last_servo_4_move) > servo_4_delay:
            if servo_4_mode == "links" and servo_4_angle < 180:
                set_servo_4_angle(servo_4_angle + servo_4_step)
            elif servo_4_mode == "rechts" and servo_4_angle > 0:
                set_servo_4_angle(servo_4_angle - servo_4_step)
            else:
                print("-> Servo_4 an Anschlag, stoppe automatisch")
                servo_4_mode = "stop"
            last_servo_4_move = now
...
#                         adjust_servo_angle(3, -10)
                    ...
                    elif msg == 'P_ein_G' and not pumpe_aktiv:
                        start_pumpe()
                        pumpe_aktiv = True
                        pumpe_startzeit = time.ticks_ms()
...
#                         stopp_pumpe()
    ...
    # Pumpe nach 3 Sekunden stoppen
    if pumpe_aktiv:
        if time.ticks_diff(time.ticks_ms(), pumpe_startzeit) >= 3000:
            stopp_pumpe()
            pumpe_aktiv = False
