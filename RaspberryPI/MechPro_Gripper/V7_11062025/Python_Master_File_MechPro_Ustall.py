...
# Anpassung der Steuerung fuer die Jellyfish Aufgabe                                  // 11.06.2025       // Johann Spielvogel
#
#
...
def on_message(client, userdata, message):
    ...
    global pumpe_aktiv, pumpe_timer
    #Servo_1------------------------------------------------------------------------------------------------
    if direction == 'SLD_1':
    ...
    # Pumpe ausgeschaltet melden Funktion
    def pumpe_ausgeschaltet_melden():
        global pumpe_aktiv
        client.publish("Ustall/Status", "Pumpe ausgeschaltet")
        print("Pumpe ausgeschaltet gemeldet")
        pumpe_aktiv = False
    #Pumpe------------------------------------------------------------------------------------------------
    if direction == 'P_ein' or direction == 'P_ein_G':
        if not pumpe_aktiv:
            ...
            if pumpe_timer is not None:
                pumpe_timer.cancel()

            # Starte neuen Timer für 3 Sekunden
            pumpe_timer = threading.Timer(3.0, pumpe_ausgeschaltet_melden)
            pumpe_timer.start()
        else:
            print("Pumpe läuft bereits, Timer wird nicht neu gestartet")
    ...
    #Jellyfish--------------------------------------------------------------------------------------------
    if direction == 'JF_z':
        print("Jellyfish_auf AKTIV")
        uart.write(b'JF_z\n')
        client.publish("Ustall/Status", "Jellyfish_auf AKTIV")

    elif direction == 'JF_a':
        print("Jellyfish_zu AKTIV")
        uart.write(b'JF_a\n')
        client.publish("Ustall/Status", "Jellyfish_zu AKTIV")

    elif direction == 'JF_S':
        print("Jellyfish_stopp AKTIV")
        uart.write(b'JF_S\n')
        client.publish("Ustall/Status", "Jellyfish_stopp AKTIV")
